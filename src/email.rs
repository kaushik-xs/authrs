...
pub async fn send_membership_invite_email(
    smtp: &SmtpConfig,
    to_email: &str,
    token: &str,
    ...
    let body = match invite_link {
        Some(link) => format!(
            "Someone requested to add your account to a new workspace. If this was you, click the link below to confirm and join:\n\n{}\n\nThis link expires in 1 hour. If you did not request this, you can safely ignore this email.",
            link
        ),
        None => format!(
            "Someone requested to add your account to a new workspace. If this was you, use the following token to confirm:\n\n{}\n\nThis token expires in 1 hour. If you did not request this, you can safely ignore this email.",
            token
        ),
    };
    ...
    let message = Message::builder()
        .from(from_addr)
        .to(to_addr)
        .subject("Confirm joining a new workspace")
        .header(ContentType::TEXT_PLAIN)
        .body(body)?;
